...
def _load_generated_files(file_map: Dict[str, str]) -> Dict[str, pd.DataFrame]:
    """Attempt to load the CSVs created by the generator page."""
...
with tab_llm:
    ...
    if sample_choice == "Compliant (5/5)":
        ...
        if not compliant.empty:
            ...
            sample_note_series = alerts_df[alerts_df["policy_id"] == policy_id]["reviewer_notes"]
            if not sample_note_series.empty:
                sample_note = sample_note_series.iloc[0]
...
with tab_export:
    ...
    with col_left:
        ...
        auditor_name = st.text_input("Lead Auditor", "")
    with col_right:
        audit_period_start = st.date_input("Period Start")
        audit_period_end = st.date_input("Period End")
